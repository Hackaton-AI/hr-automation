...
class ResumeScreeningModel:
    """
    Complete model service that handles preprocessing and prediction with proper feature dimension matching
    """

    def _init_(self, model_path="C:/Users/Ismael/Desktop/projects/hr-automation/models/resume-screening/resume_screening_model.h5",
    ...
    def debug_preprocessing_components(self):
        """Debug the loaded preprocessing components"""
        if self.preprocessing_components is None:
            self.logger.error("No preprocessing components loaded")
            return

        self.logger.info("=== DEBUGGING PREPROCESSING COMPONENTS ===")

        for key, value in self.preprocessing_components.items():
            self.logger.info(f"Key: {key}, Type: {type(value)}")

            if key == 'scaler':
                try:
                    if hasattr(value, 'mean_'):
                        feature_count = len(value.mean_)
                        self.expected_feature_count = feature_count
                        self.logger.info(
                            f"  - scaler is fitted, feature count: {feature_count}")
                    else:
                        self.logger.error("  - scaler is NOT fitted!")
                except Exception as e:
                    self.logger.error(f"  - Error checking scaler: {e}")

            elif key == 'tfidf_resume':
                try:
                    if hasattr(value, 'idf_'):
                        self.logger.info(
                            f"  - tfidf_resume is fitted, vocabulary size: {len(value.vocabulary_)}")
                    else:
                        self.logger.error("  - tfidf_resume is NOT fitted!")
                except Exception as e:
                    self.logger.error(f"  - Error checking tfidf_resume: {e}")

            elif key == 'tfidf_jd':
                try:
                    if hasattr(value, 'idf_'):
                        self.logger.info(
                            f"  - tfidf_jd is fitted, vocabulary size: {len(value.vocabulary_)}")
                    else:
                        self.logger.error("  - tfidf_jd is NOT fitted!")
                except Exception as e:
                    self.logger.error(f"  - Error checking tfidf_jd: {e}")

            elif key == 'categorical_columns':
                self.logger.info(f"  - categorical columns: {value}")

            elif key == 'feature_columns':
                if isinstance(value, list):
                    self.logger.info(
                        f"  - total feature columns: {len(value)}")
                    # Count different feature types
                    resume_features = len(
                        [col for col in value if col.startswith('resume_')])
                    jd_features = len(
                        [col for col in value if col.startswith('jd_')])
                    cat_features = len(value) - resume_features - jd_features
                    self.logger.info(
                        f"    - resume features: {resume_features}")
                    self.logger.info(f"    - jd features: {jd_features}")
                    self.logger.info(
    ...
    def initialize(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"joblib failed: {e}, trying pickle...")
    ...
    def _check_and_fix_tfidf(self):
        ...
        if not resume_fitted or not jd_fitted:
            ...
            self.logger.info("✓ TF-IDF vectorizers are properly fitted")

    def _create_fallback_tfidf(self):
        """Create fallback TF-IDF vectorizers with proper dimensions"""
        self.logger.info("Creating fallback TF-IDF vectorizers...")
        ...
        if resume_features is None or jd_features is None:
            self.logger.error("Cannot determine proper feature dimensions!")
            raise ValueError(
    ...
    def create_tfidf_features(self, resume_text, jd_text):
        ...
        try:
            ...
            self.logger.debug(f"Resume TF-IDF shape: {resume_tfidf.shape}")
            self.logger.debug(f"JD TF-IDF shape: {jd_tfidf.shape}")

            return resume_tfidf, jd_tfidf

        except Exception as e:
            self.logger.error(f"TF-IDF transformation error: {e}")
            raise
    ...
    def create_categorical_features(self, job_family, seniority):
        ...
        try:
            ...
            self.logger.error(f"Categorical features creation error: {e}")
            expected_cols = self.preprocessing_components.get(
                'categorical_columns', [])
    ...
    def create_feature_vector(self, resume_text, jd_text, job_family, seniority):
        ...
        self.logger.debug(
            f"Final feature vector shape: {feature_vector.shape}")
        self.logger.debug(
            f"Expected feature count: {self.expected_feature_count}")
